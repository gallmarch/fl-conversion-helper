...
export default function insertExtension({ store, isLegacy = true }) {
  ...
  const container = createAndInsertContainerElement(isLegacy);
...
function createAndInsertContainerElement(isLegacy) {
  // This will eventually resolve to false and we can remove the entire block
  if (isLegacy) {
  ...
  throw new Error({ message: 'Not implemented yet; waiting for beta' });
}
